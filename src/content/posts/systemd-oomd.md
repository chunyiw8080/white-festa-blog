...
pinned: false
description: 本文介绍 systemd-oomd——一个集成在 systemd 中的用户态 OOM 管理工具。文章从内核 PSI 机制入手，解释其输出指标的含义；随后说明 systemd-oomd 的三种触发场景，并与传统内核 OOM Killer 进行对比。通过配置文件详解、服务单元参数设置以及自定义 slice 的创建与资源隔离等实战示例，展示如何利用 systemd-oomd 在内存压力升高时主动释放资源。
tags: [Linux性能优化, cgroups]
category: Devops
draft: false
---

systemd-oomd 是一个用户级 OOM 杀手守护进程，集成在 systemd 中。它利用 cgroup v2 监控系统和用户单元的内存压力，在内核 OOM 杀手触发前主动终止内存占用过高的进程，旨在改善低内存场景下的系统响应速度，避免主机卡死、桌面无响应、甚至是 kernal 进入全局 OOM 风暴等情况。
